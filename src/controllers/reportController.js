...
const exportUsersPDF = async (req, res) => {
    try {
        ...
        // Título
        doc.fontSize(20).text("Relatório de Usuários", { align: "center", underline: true });
        doc.moveDown(1);
        ...
        users.forEach((user) => {
            ...
            doc.fillColor("black").text(`${user.id}    ${user.name}    ${user.email}`);
...
const exportPostPDF = async (req, res) => {
    try {
        ...
        // Título
        doc.fontSize(20).text("Relatório de Posts", { align: "center", underline: true });
        doc.moveDown(1);

        // Cabeçalho
        doc.fontSize(12).text("Id | User ID | Descrição | Imagem | Pessoa Adicionada | Localização", { underline: true });
        doc.moveDown(0.5);
        ...
        posts.forEach((post) => {
            ...
            doc.fillColor("black").text(
                `${post.id}    ${post.user_id}    ${post.description}    ${post.image || "N/A"}    ${post.add_person || "N/A"}    ${post.localization || "N/A"}`
            );
            doc.moveDown(0.5); // Espaçamento entre linhas
            isAlternate = !isAlternate;
        });
